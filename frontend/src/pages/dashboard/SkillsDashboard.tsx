    ...
    const fetchSkillsData = async () => {
      try {
        ...
        const [radarRes, masteryRes, graphRes] = await Promise.all([
          fetch("http://localhost:5000/api/dashboard/skills/radar", { headers: { Authorization: `Bearer ${token}` } }),
          fetch("http://localhost:5000/api/dashboard/skills/mastery", { headers: { Authorization: `Bearer ${token}` } }),
          fetch("http://localhost:5000/api/dashboard/skills/topic-graph", { headers: { Authorization: `Bearer ${token}` } })
        ]);
        ...
        // Check if radar data is empty or all scores are 0 (which happens for new users)
        ...
        if (isRadarEmpty) {
          ...
          setRadarData(radarJson.radar);
        }

        // Mock mastery data if empty
        if (!masteryJson.mastery || masteryJson.mastery.length === 0) {
          setMasteryData([
          ...
          setMasteryData(masteryJson.mastery);
        }
